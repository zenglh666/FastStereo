...
class PSMNet(nn.Module):
    def __init__(self, args):
        ...
        for i in range(3):
            ...
            self.fuse_conv.append(nn.Sequential(
                nn.Conv2d(inplanes, args.planes*2, kernel_size=1, stride=1, padding=0, dilation=1, bias=False),
        ...
        self.refine_conv = nn.Sequential(
            block2d(args.planes*2, kernel_size=3, stride=1, padding=args.dilation, dilation=args.dilation),
            block2d(args.planes*2, kernel_size=3, stride=1, padding=args.dilation, dilation=args.dilation),
            nn.Conv2d(args.planes*2, 1, kernel_size=1, stride=1, padding=0, dilation=1, bias=False),
        )
    ...
    def forward(self, left, right):
        ...
        for i, conv in enumerate(self.fuse_conv):
            ...
            res = self.refine_conv(conv(feature))
