...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        for fuser, classifier, regressor in zip(self.fusers, self.classifiers, self.regressers):
            ...
            preds.append(self.upsample_disp(pred, 2**self.depth, sample_type="pure"))
            summary += pred
        ...
        for i, (refinement, weight, bias) in enumerate(zip(self.refinements, self.weights, self.biases)):
            ...
            summary = self.upsample_disp(summary, 2, sample_type="pure")
            range_h_w = self.get_range(summary.size()[1], summary.size()[2], summary.device)
            flow = summary.view(summary.size()[0], summary.size()[1], summary.size()[2], 1) / (summary.size()[2] - 1)
            ...
            feature = torch.cat((refimg_fea, targetimg_fea, torch.unsqueeze(summary, 1)), dim=1)

            refine = refinement(feature)
            pred = torch.squeeze(bias(refine), 1)
            preds.append(self.upsample_disp(pred, 2**(self.depth-i-1), sample_type="pure"))
            summary += pred
