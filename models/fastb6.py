...
class PSMNet(nn.Module):
    def __init__(self, args):
        ...
        self.feature_extraction = nn.Sequential(
            ...
            block2d(args.planes, args.planes, kernel_size=5,  padding=2*args.dilation, dilation=args.dilation),
            block2d(args.planes*2, args.planes, kernel_size=5,  padding=2*args.dilation, dilation=args.dilation),
            block2d(args.planes*3, args.planes, kernel_size=5,  padding=2*args.dilation, dilation=args.dilation),
        )
        ...
        self.fuse = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            block3d(args.planes, args.planes, kernel_size=5,  padding=2*args.dilation, dilation=args.dilation),
            block3d(args.planes*2, args.planes, kernel_size=5,  padding=2*args.dilation, dilation=args.dilation),
            block3d(args.planes*3, args.planes, kernel_size=5,  padding=2*args.dilation, dilation=args.dilation),
        ) 

        inplanes = self.planes * 4

        self.classifier = nn.ConvTranspose3d(
            inplanes, 1, kernel_size=7, stride=4, padding=3, output_padding=3, dilation=1, bias=False)
