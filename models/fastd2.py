...
class PSMNet(nn.Module):
    def __init__(self, args):
        ...
        self.disparityregression = disparityregression(self.maxdisp//(2**(self.depth+1)))
    ...
    def forward(self, left, right):
        ...
        #matching
        cost = torch.zeros(
            [refimg_fea.size()[0], refimg_fea.size()[1]*2, self.maxdisp//(2**(self.depth+1)),  refimg_fea.size()[2],  refimg_fea.size()[3]],
            device=refimg_fea.device)

        for i in range(self.maxdisp//(2**(self.depth+1))):
            if i > 0 :
                cost[:, :refimg_fea.size()[1], i, :,2*i:]   = refimg_fea[:,:,:,2*i:]
                cost[:, refimg_fea.size()[1]:, i, :,2*i:] = targetimg_fea[:,:,:,:-2*i]
            else:
        ...
        for fuser, classifier, regressor in zip(self.fusers, self.classifiers, self.regressers):
            ...
            pred = self.disparityregression(pred) * 2
