    ...
    def __init__(self, args):
        ...
        for i in range(self.depth - 1):
            ...
            self.refinements.append(nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(outplanes, outplanes//2, kernel_size=1, stride=1, padding=0, dilation=1, bias=False),
                nn.BatchNorm2d(outplanes//2),
                nn.ReLU(inplace=True),
                block2d(outplanes//2, kernel_size=3, stride=1, padding=1, dilation=1),
                block2d(outplanes//2, kernel_size=3, stride=1, padding=1, dilation=1),
            ))
            self.weights.append(nn.Sequential(
                nn.Conv2d(outplanes//2, (2*self.flood + 1)*(2*self.flood + 1), 
